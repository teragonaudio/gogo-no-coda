...
static const int SizeOfEmptyFrame[2][2]=
{
	{17,9},
	{32,17},
};
...
*/
void
AddVbrFrame(int bitrate_index)
{
    int kbps = bitrate_table[RO.version][bitrate_index];
    
    addVbr(&RW.VBR_seek_table, kbps);
//    RW.nVbrNumFrames++;
}
...
//	RW.nVbrNumFrames=0;
	...
	if (nMode == MONO) {
		BE.nZeroStreamSize=SizeOfEmptyFrame[RO.version][1]+4;
	} else {
		BE.nZeroStreamSize=SizeOfEmptyFrame[RO.version][0]+4;
	}
	...
	*/
	...
	tot = (RO.sideinfo_len + LAMEHEADERSIZE);
	assert( bitrate <= 384 );

	BE.TotalFrameSize = ((RO.version+1)*72000*bitrate) / RO.out_samplerate;
	/* extra 20 bytes for LAME & version string */
	assert(BE.TotalFrameSize >= (BE.nZeroStreamSize+VBRHEADERSIZE) + 20);
	assert(BE.TotalFrameSize <= MAXFRAMESIZE );

	if ( BE.TotalFrameSize < tot || BE.TotalFrameSize > MAXFRAMESIZE ) {
		// disable tag, it wont fit
		RO.bWriteVbrTag = 0;
		return 0;
    }

	for (i=0; i<BE.TotalFrameSize; ++i)
	  add_dummy_byte(0);
	}

        RW.VBR_seek_table.sum  = 0;
        RW.VBR_seek_table.seen = 0;
        RW.VBR_seek_table.want = 1;
        RW.VBR_seek_table.pos  = 0;
        RW.VBR_seek_table.size = 400;
...
/****************************************************************************
 * CreateVbrTag: Create VBR tag 
 * Paramters:
 ****************************************************************************
*/
int CreateVbrTag(void **pTagData,size_t *pSizeResult, int nVbrScale, const size_t sizeWrite)
{
	long lFileSize;
	int nStreamIndex;
	char bbyte;
	...
	u_char pbtStreamBuffer[MAXFRAMESIZE];
	int		i;
	const char *pVBRTag = VBRTag;
	unsigned short crc = 0x00;
//        unsigned char id3v2Header[10];
//        const size_t id3v2TagSize = 0;
	gogo_thread_data *tl = RO.tl; 
		
//		FILE *fpStream = NULL;
	assert( RO.bWriteVbrTag );
	if ( RW.VBR_seek_table.pos <= 0 || (!RO.bWriteVbrTag ) )
		return -1;
	...
	pbtStreamBuffer[1] = (u_char) ( RO.out_samplerate < 16000 ) ? 0xE0 : 0xF0;
	pbtStreamBuffer[1] |= 
					(RO.version << 3) | 
					((4 - 3 /* Layer3 */) << 1) | 
					1; /* Error Protection */;

	{
		...
		bbyte = 16*BitrateIndex(bitrate,RO.version);
	}

	pbtStreamBuffer[2]= bbyte | 
			(RO.samplerate_index << 2) |
			( tl->padding << 1) |
			0;								/* 0 extension */
	...
	memset(btToc,0,sizeof(btToc));
        ...
        Xing_seek_table (&RW.VBR_seek_table, btToc);
        /* print_seeking (btToc); */

	/* Start writing the tag after the zero frame */
	nStreamIndex=BE.nZeroStreamSize;

	/* Put Vbr tag */
	pbtStreamBuffer[nStreamIndex++]=pVBRTag[0];
	pbtStreamBuffer[nStreamIndex++]=pVBRTag[1];
	pbtStreamBuffer[nStreamIndex++]=pVBRTag[2];
	pbtStreamBuffer[nStreamIndex++]=pVBRTag[3];
	...
	CreateI4(&pbtStreamBuffer[nStreamIndex],RW.frameNum );
	nStreamIndex+=4;
...
//	nStreamIndex+=4;
	...
	if( RO.bWriteLameTag ){
		...
		assert( nStreamIndex < sizeof( pbtStreamBuffer ) );
	#ifdef DEBUG_VBRTAG
		{
...
#if 1
	{
		static char s_buf[2*9*384];
		assert( BE.TotalFrameSize <= sizeof(s_buf) );
		memcpy( s_buf, pbtStreamBuffer, BE.TotalFrameSize );

		*pSizeResult	= BE.TotalFrameSize;
		*pTagData		= s_buf;
	}
#else
	{
		void	*pAllocData = malloc( BE.TotalFrameSize );
		if( pAllocData == NULL ) return -1;
		memcpy( pAllocData, pbtStreamBuffer, BE.TotalFrameSize );
...
//	FLOAT fVersion = LAME_MAJOR_VERSION + 0.01 * LAME_MINOR_VERSION;
    ...
    int enc_delay   = (1==RO.version) ? 576: 288;	//lame_get_encoder_delay(gfp);       // encoder delay
    int enc_padding = RW.num_samples_padding;		//lame_get_encoder_padding(gfp);   // encoder padding 

	//recall:	gfp->VBR_q is for example set by the switch -V 
	//			gfp->quality by -q, -h, -f, etc
	
	int nQuality		= (100 - 10 * BE.VBR_q - BE.quality);
	

	const char *szVersion	= (const char *)"LAME3.92";
	unsigned char nVBR;
	unsigned char nRevision = 0x00;
	unsigned char nRevMethod;
	unsigned char vbr_type_translator[] = {1,5,3,2,4,0,3};		//numbering different in vbr_mode vs. Lame tag

	unsigned char nLowpass		= ( ((BE.lowpassfreq / 100.0)+.5) > 255 ? 255 : (BE.lowpassfreq / 100.0)+.5 );

	ieee754_float32_t fPeakSignalAmplitude	= 0;				//TODO...
	unsigned short nRadioReplayGain		= 0;				//TODO...
	unsigned short nAudioPhileReplayGain  = 0;				//TODO...


	unsigned char nNoiseShaping			= RO.noise_shaping ;
	unsigned char nStereoMode				= 0;
	int		bNonOptimal				= 0;
	unsigned char nSourceFreq				= 0;
	unsigned char nMisc					= 0;
	unsigned long nMusicLength			= 0;
	int		bId3v1Present			= 0;//((gfp->internal_flags->tag_spec.flags & CHANGED_FLAG) && !(gfp->internal_flags->tag_spec.flags & V2_ONLY_FLAG));
	unsigned short nMusicCRC				= 0;

	//psy model type: Gpsycho or NsPsytune
	unsigned char    bExpNPsyTune	= 0;//gfp->exp_nspsytune & 1;
	unsigned char	 bSafeJoint		= 0;//(gfp->exp_nspsytune & 2)!=0;

	unsigned char	 bNoGapMore		= 0;
	unsigned char	 bNoGapPrevious	= 0;

	int		 nNoGapCount	= 0;//gfp->internal_flags->nogap_total;
	int		 nNoGapCurr		= 0;//gfp->internal_flags->nogap_current;


	unsigned char  nAthType		= 0;//gfp->ATHtype;	//4 bits.
	
	unsigned char  nFlags			= 0;

	// if ABR, {store bitrate <=255} else { store "-b"}
	int nABRBitrate	= RO.brate;//(RO.VBR == vbr_abr) ? gfp->VBR_mean_bitrate_kbps:gfp->brate;

	//revision and vbr method
	if ( RO.VBR >= 0 && RO.VBR < sizeof(vbr_type_translator))
...
//		nStereoMode = 5;
	...
	if (BE.inpFreqHz <= 32000)
		nSourceFreq = 0x00;
	else if (BE.inpFreqHz ==48000)
		nSourceFreq = 0x02;
	else if (BE.inpFreqHz > 48000)
		nSourceFreq = 0x03;
	else
...
*/
	...
	nFilesize = sizeWrite;//ftell(fpStream);
...
*/
	nMusicLength -= BE.addtagInfo.addtagLen ;
